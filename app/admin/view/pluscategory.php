       <!-- <div class="title"></div> -->
        <div class="tool">
            <section class="home">
                <div class="toggle_silebar">
                    <i class="fa-solid fa-bars"></i>
                    <div class="text">Xin chào admin</div>
                </div>
                <div class="tool_Notification">
                    <div class="tool_reply">
                        <a href="../view/index.html">
                            <i class="fa-solid fa-reply"></i>
                        </a>
                    </div>
                    <div class="too_setting">
                        <i class="fa-solid fa-gears" id="icon_seting"></i>
                        <div class="setting_information">
                            <ul class="setting_information_list">
                                <li class="setting_information_item">
                                    <a href="">
                                        <i class="fa-solid fa-user-gear"></i>
                                        <span> Thông tin admin</span>
                                    </a>
                                </li>
                                <li class="setting_information_item">
                                    <a href="">
                                        <i class="fa-solid fa-key"></i>
                                        <span> Đổi mật khẩu</span>
                                    </a>
                                </li>
                                <li class="setting_information_item">
                                    <a href="">
                                        <i class="fa-regular fa-trash-can"></i>
                                        <span>Xóa bộ nhớ cache</span>
                                    </a>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <div class="tool_bell">
                        <i class="fa-solid fa-bell" id="icon_bell"></i>
                        <div class="notification_contatc">
                            <ul class="notification_contatc_list">
                                <li class="notification_contatc_item">
                                   <span>Thông báo</span>
                                </li>
                                <li class="notification_contatc_item">
                                    <a href="">
                                        <i class="fas fa-envelope"></i>
                                        <span>liên hệ</span>
                                    </a>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <div class="tool_logout">
                        <a href="../admin/login.html">
                            <i class="fa-solid fa-right-from-bracket"></i>
                            <span>đăng xuất</span>
                        </a>
                    </div>
                </div>
            </section>
            <!-- --------------bẳng điều khiển-------------------- -->
            <!-- <section class="dashboard">
                <header class="dashboard_header">
                    <p>bảng điều khiển</p>
                </header>
                <nav class="dashboard_header_tool">
                    <div class="configuration_website">
                        <a href="configuration.html">
                            <div class="configuration_website_icon">
                                <i class="fas fa-cogs"></i>
                            </div>
                            <div class="configuration_website_txt">
                                <h5>Cấu hình website</h5>
                                <span>xem thêm</span>
                            </div>
                        </a>
                    </div>
                    <div class="account">
                        <a href="account.html">
                            <div class="account_website_icon">
                                <i class="fas fa-user-cog"></i>
                            </div>
                            <div class="account_website_txt">
                                <h5>tài khoản</h5>
                                <span>xem thêm</span>
                            </div>
                        </a>
                    </div>
                    <div class="change_password">
                        <a href="changepass.html">
                            <div class="change_password_website_icon">
                                <i class="fas fa-key"></i>
                            </div>
                            <div class="change_password_website_txt">
                                <h5>đổi mật khẩu</h5>
                                <span>xem thêm</span>
                            </div>
                        </a>
                    </div>
                    <div class="contact_letter">
                        <a href="contact_letter.html">
                            <div class="contact_letter_website_icon">
                                <i class="fas fa-address-book"></i>
                            </div>
                            <div class="contact_letter_website_txt">
                                <h5>thư liên hệ</h5>
                                <span>xem thêm</span>
                            </div>
                        </a>
                    </div>
                </nav>
            </section> -->
            <!-- -------------------kết thúc bảng điều khiển-------------- -->
             <!-- <section id=" level_1_business_category">
                <header class="level_1_business_category_header">
                    <a href="../admin/index.html">bảng điều khiển</a>
                    /
                    <a href="../admin/catologylv1.html">danh mục cấp 1</a>
                </header>
                <nav class="level_1_business_category_nav">
                    <form action="" method="post">
                        <div class="input_tool">
                            <div class="add_product">
                               <a href="">
                                    <i class="fa-solid fa-plus"></i>
                                    <span>Thêm mới</span>
                               </a>
                            </div>
                            <div class="dellet_product">
                                <a href="">
                                    <i class="fa-regular fa-trash-can"></i>
                                    <span>Xóa tất cả</span>
                               </a>
                            </div>
                            <div class="search_product">
                                <input type="search" name="txtsearch" id="txtsearch" placeholder="Tìm kiếm">
                                <input type="submit" value="Nhập" onclick="return btnsearch()" id="search">
                                <div class="error"></div>
                            </div>
                        </div>
                    </form>
                    <div class="category">
                        <div class="category_txt">
                            <span class="category_txt_name">Danh mục cấp 1</span>
                        </div>
                        <table class="table_category">
                            <tr class="table_tr">
                                <th>STT</th>
                                <th>Tiêu đề</th>
                                <th>Nổi bật </th>
                                <th>Hiển thị</th>
                                <th>Thao tác</th>
                            </tr>
                            <tr class="table_tr">
                                <td>STT</td>
                                <td><span>Sản phẩm nhập khẩu</span></td>
                                <td><input type="checkbox" name="txtoutstanding" id="txtoutstanding"> </td>
                                <td><input type="checkbox" name="txtdisplay" id="txtdisplay"> </td>
                                <td class="table_linl"><a href="" class="edit"><i class="fa-solid fa-pen-to-square" class="delete"></i></a>|<a href=""><i class="fa-regular fa-trash-can"></i></a></td>
                            </tr>
                        </table>
                    </div>
                </nav>
             </section> -->
             <!-- ---------phần thêm sản phẩm------------- -->
              <!-- -------kết thúc phần thêm sản phẩm-------- -->
               <section id="pluscatalog">
                    <header class="level_1_business_category_header">
                        <a href="../admin/index.html">bảng điều khiển</a>
                        /
                        <a href="../admin/catologylv1.html">Thêm danh mục cấp 1</a>
                    </header>
                    <nav class="level_1_business_pluscatalog">
                        <form action="" method="post">
                            <div class="tool_plus">
                                <div class="tool_save" onclick="return save_catologo_lv1()">
                                    <a href="" >
                                        <i class="fa-solid fa-floppy-disk"></i>
                                        <span>Lưu sản phẩm</span>
                                    </a>
                                </div>
                                <div class="tool_reset">
                                    <i class="fas fa-undo"></i>
                                    <input type="reset" value="Làm lại">
                                </div>
                                <div class="tool_out" onclick=" return logout_plus()">
                                    <a  href="">
                                        <i class="fa-solid fa-right-from-bracket"></i>
                                        <span>Thoát</span>
                                    </a>
                                </div>
                            </div>
                            <div class="table_form">
                               <div class="notification-plus">
                                    <p class="text">Đường dẫn</p>
                                    <p id="erorr_notification_plus"></p>
                               </div>
                               <table class="table_plus_catolog">
                                    <tr>
                                        <td><span>Đường dẫn mẫu(vi)</span></td>
                                        <td>
                                            <input type="text" name="txtpath" id="txtpath" placeholder="Đường dẫn (vi)" maxlength="20">
                                        </td>
                                    </tr> 
                               </table>
                               <table class="table_plus_catolog">
                                    <div class="title_txt">
                                        <p>nội dung danh mục cấp 1</p>
                                    </div>
                                <tr>
                                    <div class="checkbox_table_display">
                                        <div class="checkbox_tabel_outstanding">
                                            <span>Nổi bật</span>
                                            <input type="checkbox" name="chkoutstanding" id="chkoutstanding">
                                        </div>
                                        <div class="checkbox_tabel_outstanding">
                                            <span>Hiển thị</span>
                                            <input type="checkbox" name="chkdisplay" id="chkdisplay">
                                        </div>
                                    </div>
                                </tr>
                                <tr class="tr_form">
                                    <td><p>Tiêu đề</p></td>
                                    <td><input type="text" name="txttitle" id="txttitle" placeholder="Tiêu đề (vi)"></td>
                                </tr>
                               </table>
                            </div>
                        </form>
                    </nav>
               </section>
        </div>